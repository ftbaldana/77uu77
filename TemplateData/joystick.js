...
function __init_joystick_div()
{
	...
	div_style.touchAction = "manipulation";
...
var JoyStickButton = function( attrs )
{
	...
	if(attrs.img){
		...
		img.style.width = this.radius * 2 - 20 + "px";
		img.style.height = this.radius * 2 - 20 + "px";
	...
	if ( attrs.funcDown )
	{
		if ( this.mouse_support )
		{
			this.bind( 'mousedown', attrs.funcDown );
		}
		this.bind( 'touchstart', attrs.funcDown );
		this.bind( 'touchend', attrs.funcUp );		
	}
...
function InitJoyStick(){
	...
	var camera = new JoyStickButton({
		...
		funcUp: (e)=>{
			window.dispatchEvent(new KeyboardEvent('keyup',{'keyCode': 70}));		
		}
	});
	...
	var nitro = new JoyStickButton({
		...
		funcUp: (e)=>{
			window.dispatchEvent(new KeyboardEvent('keyup',{'keyCode': 16}));		
		}
	});
}

function SetJoyStickVisible(value){
	if(value)
		JOYSTICK_DIV.style.visibility = 'visible';
	else	
		JOYSTICK_DIV.style.visibility = 'hidden';	
}